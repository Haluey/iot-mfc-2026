...
Win32API 기반으로 GUI를 라이브러리화 -> Win32 API를 C++ 클래스로 감싸놓은 라이브러리
...
```cpp
HWWD wnd = CreateWindow(...);
```

MFC는 각 컨트롤을 C_로 미리 만들어 놓음
```cpp
CWnd
CDialog
CButton
...
- 리소스 UI
    - **MFCBasic.rc** : 가장 중요한 리소스 파일. 다이얼로그 UI, 메뉴, 아이콘, 툴바, 문자열 테이블, 버전, 비트맵 등
...
리소스 뷰(Ctrl + Shift + E) 전환 뒤, Dialog의 IDD_MFCBASIC_DIALOG 클릭하면
...
- Dialog Based는 리소스에서 확인 가능하지만, SDI/MDI는 전체 화면을 확인할 메뉴/디자인 뷰가 없음
...
- MFCBasic.h 소스
    ...
    ```cpp
    class MFCBasicApp : public CWinApp
    ```

- CWinApp은 MFC가 제공하는 클래스이고 우리가 만든 CMFCBasicApp은 이를 상속한 자식 클래스

##### WinMain이 없다! 대신 InitInstance() 가 중요!
...
- MFCBasic.cpp 소스
    ...
    {
        CWinApp::InitInstance();    // 부모 객체도 초기화한다
        ...
    ```
...
- MFC는 `CMFCBasicDlg dlg;` C++ 객체로 코딩. 실제 MFC 내부에 HWND로 구성되어 있고, 이걸 MFC가 핸들링
- MFC CWnd 는 Win32 API의 HWND를 C++ 클래스로 감싸 놓은 것일 뿐
...
- MFCBasicDlg.cpp 소스
    - 버튼을 추가하고 더블클릭해서 이벤트 함수가 생성되면 ON_BN_CLICKED... 자동 추가됨
...
- C++ MFC로 위 코드만큼 줄였다는 것이 장점
    - Win32 API -> WndProc + WM_MESSAGES + HWND
    - MFC -> CDialogEx + Message Map + CButton

#### Resource Editor

Visual Studio가 제공하는 **GUI 디자이너** 기능. C# WinForms의 Form Designer와 같은 역할
...
- STATIC, EDIT, BUTTON 3개로 구성

    ![alt text](images/image-13.png)

- MFC에서 추천하는 이름은 일반적으로 IDC_Prefix 사용 
...
- ID Edit 컨트롤 선택 상태 > Context Menu > 변수 추가
...
- MFCControlsDlg.cpp에 DoDataExchange() 함수에 추가된 내용 확인
...
- OnBnClickedBtnLogin() 함수 내 작성 내용 삭제
...
- GetDlgItemText()가 컨트롤 별로 수행할 필요가 없음
- UpdateData(TRUE) : 데이터 읽기
- UpdateData(FALSE) : 데이터 쓰기

여기까지가 DDX Value로 처리하는 방식

##### DDX Control

변수값이 아닌 컨트롤 자체를 제어하고자 할 때 사용

- 로그인을 성공하고 난 뒤 아이디 입력창 비활성화
- 포커스 이동
...
- 로그인 버튼, m_btnLogin로 변수 추가
- DoDataExchange() 함수에 추가 확인
...
Dialog Data Validation. 제대로 된 입려값이 들어갔는지 검증
...
- 소스 파일
    ...
    - 문서/뷰 아키텍처 선택 시 - ClassView, FileView, MFCSdiMenuTestDoc, MFCSdiMenuTestView, OutputWnd, PropertiesWnd, ViewTree 7개 파일
...
- 기능 축소로 만들어졌을 때 파일 구조
    - ChildView.cpp : 메인프레임 중앙 관리 소스 ★★★★★
    - MainFrm.cpp : 내용 생략 ★★★★★
    - MFCSimpleSdi.cpp : 프로그램 시작점 ★★★★★
    - pch.cpp, framework.h 등 : 위와 동일

    ![alt text](images/image-39.png)

#### MainFrame

- MainFrm.cpp : 메뉴, 툴바, 상태바, ChildView 외 추가적 클래스뷰, 파일뷰, 출력창, 속성창을 관리하는 영역
- PreCreateWindow() : 윈도우 창 만들어지기 직전 호출되는 함수. 기존 설정되어 있던 속성을 변경하고자 할 때
- OnCreate() : 창 생성, 툴바 생성, 상태바 생성(추가적인 창도 생성)
...
IDR_MAINFRAME 더블 클릭(심플 프로젝트에서는 메뉴가 다름)
...
- 버튼 클릭이나 메뉴 클릭 등 메시지가 발생하는 기능들은 메시지맵에 자동 추가
...
- 멤버 변수 추가 `bool m_bChecked = false`
- 체크 메뉴 이벤트 처리기 추가
    ...
    void CMainFrame::OnMenuCheck()
    {
        ...
        // TODO : 체크가 되었을 때 처리로직과 체크해제 시 로직분리 작성
    }
    ```
...
- 메뉴에 `-` 입력하면 가로줄 추가됨
- 단축키는 추후 학습 필요(Accelerator > IDR_MAINFRAME 참고)

##### ChildView

- Win32 API에서 WM_PAINT와 같은 역할
...
- OnPaint()에 진행
    ...
    dc.LineTo(280, 80);	// 좌표까지 직선 그리기

    dc.Rectangle(100, 140, 300, 320);
    dc.Ellipse(100, 140, 300, 320);
...
- MouseMove 실행결과
    ...
    void CChildView::OnMouseMove(UINT nFlags, CPoint point) {
        ...
        GetParent()->SetWindowText(str);	// 부모창(MainFrame)의 제목표시줄에 str을 할당
...
- 마우스 클릭 위치 저장변수 m_ptClick 추가
    ...
    ```cpp
    private:
	    Point m_ptClick;	// 클릭한 위치 기억 변수
    ```

- OnLButtonDown에 아래 코드 추가
...
- Process와 Thread 차이
    - Process : 하나의 실행파일
    - Thread : 실행파일 내 CPU가 실제로 실행하는 작업 흐름. 시분할로 여러개 나눠서 실행
...
- 실행이 잘될 수 있고, 안될 수도 있음
- Main Thread가 UI를 관리하는데 사용자 스레드가 프로그레스바(UI)를 제어하고 있음
- 직접 제어하지 말고 메인 스레드에게 UI 변경을 요청하도록 변경

- 새로운 윈도우 메시지 생성

    ```cpp
    #define WM_PROGRESS_UPDATE (WM_USER + 1)	// 프로그레스바 위해서 새로 만드는 메시지
    ```

- 다이얼로그 메시지 맵에 ON_MESSAGE 추가
...
- 결론 : UI 스레드(메인 스레드)와 사용자 스레드는 분리할 것
...
- [X] Dialog Based MFC
- [X] CWinApp / CDialogEx 이해
- [X] Resource Editor
- [X] Button / Static / Edit / CheckBox / Radio Button 컨트롤 학습
- [X] 컨트롤 사용 간단 프로젝트
- [X] Message Map 이해
- [X] 이벤트 처리 방법 이해
- [X] 컨트롤 값 읽기 / 쓰기
